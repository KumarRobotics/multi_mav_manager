...
tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 4; roslaunch kr_multi_mav_manager multi_mav_manager.launch odom_topic:=${ODOM_TOPIC} config_path:=$HOME/.ros/" Enter
...
do
  ...
  tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch mrsl_quadrotor_launch spawn.launch use_frame_prefix:=true mav_type:=${MAV_TYPE} mav_name:=${MAV_NAME} x:=${POS_X} y:=${POS_Y} z:=${POS_Z} Y:=${POS_YAW}" Enter
  tmux split-window -t $SESSION_NAME
  ...
  tmux split-window -t $SESSION_NAME
  tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch kr_multi_mav_manager robot_tf_pub.launch mav_name:=${MAV_NAME} odom_topic:=${ODOM_TOPIC}" Enter
  tmux split-window -t $SESSION_NAME
  ...
  tmux split-window -t $SESSION_NAME
  tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; rosrun kr_trackers waypoints_to_action.py __ns:=${MAV_NAME}" Enter
  tmux split-window -t $SESSION_NAME
  tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; rosrun kr_trackers twist_to_velocity_goal.py __ns:=${MAV_NAME}" Enter
  tmux split-window -t $SESSION_NAME
  tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; cd $(rospack find kr_mav_launch)/scripts/; ./takeoff.sh ${MAV_NAME}"
  tmux select-layout -t $SESSION_NAME even-horizontal
