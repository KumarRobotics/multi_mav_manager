...
do
  ...
  tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch mrsl_quadrotor_launch spawn.launch mav_type:=${MAV_TYPE} mav_name:=${MAV_NAME} x:=${POS_X} y:=${POS_Y} z:=${POS_Z} Y:=${POS_YAW}" Enter
  tmux split-window -t $SESSION_NAME
