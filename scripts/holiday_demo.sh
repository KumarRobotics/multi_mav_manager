...
rosservice call /multi_mav_services/goFormCircle "{roll: 0.0, pitch: 0.0, yaw: 0.0, center: [6.5, 0.0, 1.75], spacing: 1.0}"
sleep 1

read -p "Press [Enter] to Rotate"
rostopic pub --once /rotate_on std_msgs/Bool "data: true"
sleep 1

read -p "Press [Enter] to form circle. We can make them go into formation"
rosservice call /multi_mav_services/goFormCircle "{roll: 0.0, pitch: 0.3, yaw: 0.0, center: [6.5, 0.0, 0.75], spacing: 1.0}"
sleep 1

read -p "Press [Enter] to form tree"
rosservice call /multi_mav_services/goFormAngle "{param_names: ['angle_degrees'], param_vals: [70], roll: 0.0, pitch: 1.59, yaw: 0.0, center: [6.5, 0.0, 2.0], spacing: 0.65}"
sleep 1
...
rostopic pub --once /rotate_on std_msgs/Bool "data: false"
sleep 1

read -p "Press [Enter] to form circle. We can make them go into formation"
rosservice call /multi_mav_services/goFormCircle "{roll: 0.0, pitch: 0.0, yaw: 0.0, center: [6.5, 0.0, 0.0], spacing: 1.0}"
sleep 1
