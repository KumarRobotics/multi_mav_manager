...
cp $(rospack find multi_mav_manager)/config/dragonfly/multi_mav_manager_dragonfly.yaml ~/.ros/multi_mav_manager.yaml
for id in ${MAV_IDS[*]}
do
...
tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 3; roslaunch multi_mav_manager dragonfly_manager.launch odom_topic:=odom_tag config_path:=$HOME/.ros/" Enter
tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 3; roslaunch multi_mav_manager multi_dragonfly_gs.launch" Enter
